...
export default function App() {
  ...
  const getGeminiClient = () => {
    const apiKey = userSettings?.geminiApiKey || (import.meta.env.VITE_GEMINI_API_KEY as string) || '';
    if (!apiKey) {
      console.warn("Gemini API Key missing. Please set it in settings or environment.");
    }
  ...
  const testGeminiKey = async () => {
    ...
    try {
      ...
      if (text) {
        ...
        setTestResult({ success: false, message: 'Error communicating with API.' });
      }
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    try {
      ...
      let assistantMsgContent = response.text() || "";
        ...
        for (const call of functionCalls) {
          ...
          if (name === 'suggest_task') {
            ...
            await addTask(
              ...
              '', 
              typedArgs.taskType || 'normal', 
              typedArgs.question
            );
            assistantMsgContent += `\n\n✅ Task "${typedArgs.text}" was added.`;
            ...
            const { type, taskTexts } = typedArgs;
            const newTaskIds = [];
  ...
  const addTask = async (
    ...
    try {
      ...
      // Trigger background audio generation (Optional: only if possible client-side or omitted if avoiding local keys)
      // For GitHub Pages, we use build-in speech synthesis anyway for the assistant.
      // But we can still save audio feedback if we generate it. 
      // Omitted for simplicity of deployment unless requested.
  ...
  const submitProof = async (file: File) => {
    ...
    try {
      // 1. Upload to Cloudinary (using unsigned upload for client-side)
      const formData = new FormData();
      formData.append('file', file);
      formData.append('upload_preset', (import.meta.env.VITE_CLOUDINARY_UPLOAD_PRESET as string) || 'shate_preset');
      ...
      // 2. Analyze with Gemini directly in client
      const genAI = getGeminiClient();
